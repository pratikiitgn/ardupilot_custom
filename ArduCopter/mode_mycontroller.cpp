...
//////////////////////////////// For PWM test ////////////////////////////////
    ...
    H_yaw   = 360.0-(ahrs.yaw_sensor)   / 100.0;     // degrees ;
    tstart  = AP_HAL::millis();
    // z_des  = copter.barometer.get_altitude() * 100.0f;
    z_des_init     = inertial_nav.get_altitude()/100.0;
    return true;
}
...
void ModeMyController::custom_PID_controller(float des_phi, float des_theta, float des_psi,float des_phi_dot, float des_theta_dot, float des_psi_dot, float des_z, float des_z_dot){
    ...
    float Kp_phi      = 0.020;   // 0.09 (out 1) // 0.09 (testbed)
    float Kp_theta    = 0.025;   // 0.15 (out 1) // 0.15 (testbed)
    float Kp_psi      = 0.1;    // 0.3 (out 1)  // 0.3  (testbed)

    float Kd_phi      = 0.4;   // 0.25 (lab 1) // 2.0 (out 1) 1.6 (testbed)
    float Kd_theta    = 0.4;    // 0.5 (lab 1) // // 3.0 (out 1) 2.5 (testbed)
...
///////////////////// Altitude controller /////////////////////
    ...
    float z_dot = inertial_nav.get_velocity().z ;
...
void ModeMyController::custom_PD_controller(float des_phi, float des_theta, float des_psi,float des_phi_dot, float des_theta_dot, float des_psi_dot, float des_z, float des_z_dot){
    ...
    float Kp_phi      = 0.05;   // 0.09 (out 1) // 0.09 (testbed)
    float Kp_theta    = 0.10;   // 0.15 (out 1) // 0.15 (testbed)
    float Kp_psi      = 0.3;    // 0.3 (out 1)  // 0.3  (testbed)

    float Kd_phi      = 3.0;    // 2.0 (out 1) 1.6 (testbed)
    float Kd_theta    = 4.0;    // 3.0 (out 1) 2.5 (testbed)
...
///////////////////// Altitude controller /////////////////////
    ...
    // copter.barometer.get_climb_rate()
